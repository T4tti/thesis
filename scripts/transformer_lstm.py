...
# 1. Váº½ 2 biá»ƒu Ä‘á»“ biá»ƒu diá»…n Bar charts (1 hĂ ng, 2 cá»™t)
...
# -----------------------------------------------------------------------------
# 2. TĂ¡ch riĂªng biá»ƒu Ä‘á»“ Correlation Heatmap
# Lá»±a chá»n 20 features sá»‘ thá»±c (loáº¡i bá» binary_rating) Ä‘á»ƒ quan sĂ¡t chi tiáº¿t nháº¥t vĂ²ng TÆ°Æ¡ng quan
num_cols_all = df.select_dtypes(include=[np.number]).columns.tolist()
num_cols_corr = [c for c in num_cols_all if c not in ['binary_rating']][:20] 
corr = df[num_cols_corr].corr()

plt.figure(figsize=(14, 10)) # Äá»‹nh cáº¥u hĂ¬nh áº£nh to, dá»… review
sns.heatmap(
    corr, 
    cmap='coolwarm', 
    center=0, 
    annot=True,         # Báº­t chi tiáº¿t sá»‘ liá»‡u correlation trĂªn tá»«ng Ă´
    fmt=".2f",          # LĂ m trĂ²n 2 chá»¯ sá»‘ tháº­p phĂ¢n
    linewidths=0.5,     # Táº¡o Ä‘Æ°á»ng lÆ°á»›i má»ng nĂ©t cho cĂ¡c Ă´
    annot_kws={"size": 8} # Thu nhá» font size sá»‘ Ä‘á»ƒ trĂ¡nh bá»‹ Ä‘Ă¨ náº¿u chá»¯ quĂ¡ dĂ i
)
...
# Shared loss is fixed and excluded from hyperparameter search.
def build_training_criterion(config, n_classes):
    del config, n_classes
    return build_loss(protocol=LOSS_PROTOCOL, ordinal_lambda=ORDINAL_LAMBDA)
print('build_training_criterion uses the fixed two-tier protocol.')
...
AUX_TRANSITION_WEIGHT_MAX = 0.0
AUX_TRANSITION_WEIGHT_MIN = 0.0
...
print(f'Loss mode: {LOSS_PROTOCOL} | ordinal_lambda={ORDINAL_LAMBDA:.2f}')
...
if SHAP_ENABLED:
    ...
    with torch.no_grad():
        ...
        probs_explain = torch.softmax(logits_explain.float(), dim=1).cpu().numpy()
