...
CONFIG = {
    ...
    # Minority support parameters
    'priority_class_labels': [0, 1, 2, 3, 4, 5, 6, 18, 19, 20, 21],
    'priority_boost_multiplier': 2.0,
    'priority_min_count': 220,
    'priority_max_oversample_factor': 200.0,
    'max_synthetic_ratio': 0.45,
    ...
    'timegan_gamma': 1,
    # Eligibility controls for per-class TimeGAN
    'min_windows_per_class_for_timegan': 30,
...
if CONFIG['timegan_use_gpu'] and tf is not None:
    try:
        ...
        if len(gpus) > 0:
            ...
            timegan_gpu_runtime_note = f"âœ“ TensorFlow GPU devices: {[gpu.name for gpu in gpus]}"
        else:
            CONFIG['timegan_use_gpu'] = False
            timegan_gpu_runtime_note = "â  No TensorFlow GPU found, fallback to CPU"
    except Exception as e:
        CONFIG['timegan_use_gpu'] = False
        timegan_gpu_runtime_note = f"â  TensorFlow GPU runtime check failed, fallback to CPU: {e}"
else:
    CONFIG['timegan_use_gpu'] = False
    if tf is None:
        timegan_gpu_runtime_note = "â  TensorFlow not available, TimeGAN cannot run until dependencies are fixed"
    else:
...
if not TIMEGAN_AVAILABLE:
    print("\nâ  TimeGAN class is not importable. Please re-run dependency cell and restart kernel.")
